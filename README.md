...
Luigi was built at [Spotify](http://www.spotify.com/), mainly by Erik Bernhardsson and Elias Freider.
...
This example is also available in the *examples/* directory together with some tools to generate fake data.
...
* *HdfsTarget* is a built in class that makes it easy to read/write from/to HDFS. It also makes all file operations atomic, which is nice in case your script crashes for some reason.
...
Now, provided you have a bunch of input files in /var/tmp/text/ (you can generate them using examples/generate\_input.py), try running this using eg
    ...
    $ python wordcount.py AggregateArtists --local-scheduler --date-interval 2012-06
...
Luigi comes with Hadoop support built in, and here is how this would look like, replacing the class above
...
class AggregateArtistsHadoop(luigi.hadoop.JobTask):
    ...
    def output(self):
        return luigi.HdfsTarget("data/artist_streams_%s.tsv" % self.date_interval) # TODO: hdfs
...
The most interesting thing here is that this task (*Top10Artists*) defines a dependency on the previous task (*AggregateArtists*). This means that if the output of *AggregateArtists* does not exist, the task will run before *Top10Artists*.
    ...
    $ python wordcount.py Top10Artists --local-scheduler --date-interval 2012-06

This will run both tasks.

### Step 3 - Insert into Postgres

This mainly serves as an example of a specific subclass *Task* that doesn't require any code to be written:
...
Sometimes it's useful to have abstract tasks that mutate arbitrary input. You can do this by passing *Task* instances as parameters to other Tasks.
