...
def _partition_tasks(worker):
    ...
    set_tasks["upstream_unverifiable_dependency"] = set()
...
def _depth_first_search(set_tasks, current_task, visited):
    ...
    if current_task in set_tasks["still_pending_not_ext"]:
        ...
        upstream_unverifiable_dependency = False
        ...
        if not upstream_failure and not upstream_missing_dependency and \
                not upstream_run_by_other_worker and not upstream_unverifiable_dependency and \
...
_ORDERED_STATUSES = (
    ...
    "upstream_unverifiable_dependency",
    "unknown_reason",
)
