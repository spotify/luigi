...
class PostgresTarget(luigi.Target):
    ...
    marker_table = luigi.interface.get_config().get('postgres', 'marker-table', 'table_updates')
...
# everything below will rarely have to be overridden
    ...
    def run(self):
        ...
        tmp_dir = luigi.interface.get_config().get('postgres', 'local-tmp-dir', None)
        ...
        for row in self.rows():
            ...
            rowstr = '\t'.join(self.map_column(val) for val in row)
