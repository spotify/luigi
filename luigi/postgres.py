...
class MultiReplacer(object):
    ...
    Substituted parts will not be matched against other replace patterns,
    as opposed to when using multipass replace.
    The order of the items in the replace_pairs input will dictate
    replacement precedence.

    Constructor arguments:
    replace_pairs -- list of 2-tuples which hold strings to
                     be replaced and replace string
    ...
    def __init__(self, replace_pairs):
        # make a copy in case input is iterable
        replace_list = list(replace_pairs)
...
class PostgresTarget(luigi.Target):
    ...
    def touch(self, connection=None):
        ...
        transaction will be aborted and the connection reset.
    ...
    def create_marker_table(self):
        ...
        Using a separate connection since the transaction
...
class CopyToTable(luigi.Task):
    ...
    To customize how to access data from an input task,
    override the `rows` method with a generator that yields
    each row as a tuple with fields ordered according to `columns`.
    ...
    def exists_query(self):
        """Overload with optional query that determines if data exists

        This can be used to prevent double imports in case a marker table
        isn't used or if it has been emptied.

        If the query returns at least one row, data is deemed to exist.
        """
        return NotImplemented

    # use `columns` to specify the columns that are to be inserted
    # overload this in subclasses with either:
    # column names of columns to import:
    ...
    # container of values that should be inserted as NULL values
    null_values = (None,)

    # how columns are separated in the file copied into postgres
    column_separator = "\t"

    def rows(self):
        """Tuples or lists corresponding to each row to be inserted """
    ...
    def create_table(self, connection):
        ...
        if len(self.columns[0]) == 1:
            ...
            raise NotImplementedError(
                """create_table() not implemented for %r and
columns types not specified""" % self.table)
            ...
            query = "CREATE TABLE {table} ({coldefs})".format(
    ...
    def map_column(self, value):
        ...
        Default behaviour is to escape special characters
        and identify any self.null_values
        """
    ...
    def update_id(self):
        """Used as a unique identifier for this insert on this table."""
        return self.task_id
...
# everything below will rarely have to be overridden
    ...
    def init_copy(self, connection):
        """ Override to perform custom queries.
            ...
            Any code here will be formed in the same transaction
            as the main copy, just prior to copying data.
            Example use cases include truncating the table or removing
            all data older than X in the database to keep a
            rolling window of data available in the table.
        """
...
(column string, type string) tuples (was %r ...)""" %
    ...
    def _create_data_file(self):
         # transform all data generated by rows() using map_column
        # and write data to a temporary file for import using postgres COPY
    ...
    def _attempt_write(self, data_file, connection):
        ...
        for attempt in xrange(2):
            try:
                ...
                data_file.seek(0)
                self.copy(cursor, data_file)
            except psycopg2.ProgrammingError, e:
                if (
                    e.pgcode == psycopg2.errorcodes.UNDEFINED_TABLE
                    and attempt == 0
                ):
    ...
    def _confirmed_exists(self, connection):
        exists_query = self.exists_query()
        if exists_query is NotImplemented:
            return False  # trust update marker
        c = connection.cursor()
        try:
            ...
            if (e.pgcode == psycopg2.errorcodes.UNDEFINED_TABLE):
                connection.reset()
                return False  # table doesn't even exist
            raise
        return c.fetchone() is not None  # there is data, marker table lied!

    def run(self):
        """Inserts data generated by rows() into target table.

        If the target table doesn't exist, self.`create_table` will
        be called to attempt to create the table.
        ...
        if self._confirmed_exists(connection):
            logger.info("""Data already exists according to exists query.
Touching output marker and ignoreing data insertion""")
        else:
            data_file = self._create_data_file()
            self._attempt_write(data_file, connection)
            data_file.close()
